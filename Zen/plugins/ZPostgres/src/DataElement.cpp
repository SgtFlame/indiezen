...
{
    return m_pElement->getUnderlyingType();
}
...
{
    return m_pElement->getStringValue();
}
...
{
    return m_pElement->getRealValue();
}
...
{
    return m_pElement->getInt64Value();
}
...
{
    m_pElement->setInt64Value(_value);
}
...
{
    return m_pElement->getDateTimeValue();
}
...
DataElement::getDBString() const
{
    ...
    stream << m_pElement->getDBString();

    if (getUnderlyingType() == DATETIME)
    {
        stream << "::TIMESTAMP";
    }
